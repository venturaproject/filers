...
const DateInput: React.FC<DateInputProps> = ({ value, onChange }) => {
  ...
  const handleBlur =
    (field: keyof DateParts) => (e: React.FocusEvent<HTMLInputElement>): void => {
      if (!e.target.value) {
    ...
    (field: keyof DateParts) => (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === 'ArrowUp') {
        ...
        if (field === 'day') {
          if (date[field] === 1) {
            newDate.month -= 1
            if (newDate.month === 0) { newDate.month = 12; newDate.year -= 1 }
  ...
  return (
    <div className="flex border rounded-lg items-center text-sm px-1">
      <input
        ...
        onFocus={(e) => { if (window.innerWidth > 1024) e.target.select() }}
