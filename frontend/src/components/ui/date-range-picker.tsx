...
import { ChevronUp, ChevronDown, Check, CalendarIcon } from 'lucide-react'
      ...
      <PopoverTrigger asChild>
        <Button size="sm" variant="outline" className="h-7 gap-1">
          ...
          <div className="text-right">
            <div>
              {`${formatDate(range.from)}${range.to != null ? ' – ' + formatDate(range.to) : ''}`}
            </div>
            {rangeCompare != null && (
              <div className="opacity-60 text-xs -mt-1">
                vs. {formatDate(rangeCompare.from)}
                {rangeCompare.to != null ? ` – ${formatDate(rangeCompare.to)}` : ''}
              </div>
            )}
          </div>
                      ...
                      <DateInput
                        ...
                        onChange={(date) => {
                          if (rangeCompare) {
                            ...
                            setRangeCompare((prev) => ({ ...prev, from: date, to: compareToDate }))
                          } else {
