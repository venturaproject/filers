...
export function toIntlDateString(date: DateLike | null | undefined): string {
  if (!dateFormatter) {
  ...
  return _date instanceof Date ? dateFormatter.format(_date) : String(date ?? "")
}
...
export function toIntlTimeString(date: DateLike | null | undefined, time: string | null | undefined): string | undefined {
  if (!time) {
    return ""
  }
